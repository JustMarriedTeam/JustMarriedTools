...
BUILDER_IMAGE_NAME=${COMPONENT_NAME}-builder
APP_IMAGE_NAME=justmarried/${COMPONENT_NAME}
...
print_header "BUILD STEP 1 (Building container for building ${APP_IMAGE_NAME})"

docker build \
  -f containers/build/Dockerfile \
  -t ${BUILDER_IMAGE_NAME} . \
  || exit_with_error "Could not build container for building ${APP_IMAGE_NAME}"
